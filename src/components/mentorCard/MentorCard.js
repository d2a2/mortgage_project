...
const MentorCard = () => {
	...
	const { loading, error, getAllMentors, serverUrl } = useMortgageService();
	...
	const onRequest = () => {
		getAllMentors().then(onMentListLoaded);
	};
	...
	function renderItems(arr) {
		const items = arr.mentors?.map((item, index) => {
			const nestedItems = item.skills.map((skill, index) => {
				return (
					<div className="mentor-card__item" key={index}>
						<img
							className="mentor-card__item-img"
							src={serverUrl + skill.icon}
							alt="skill icon"
						/>
						<div className="mentor-card__item-text">{skill.name}</div>
					</div>
				);
			});
				...
				<div className="mentor-card__body" key={index}>
					<div className="mentor-card__body--row">
						...
						<div className="mentor-card__action">
							<a href={item.videoUrl} target="_blank" className="play">
					...
					<div className="mentor-card__info title--h5">
						{item.nestedArrayTitle}
						{nestedItems}
					</div>
				</div>
			);
		...
		return <div className="mentors__cards">{items}</div>;
	}
		...
		<section className="mentors">
			<div className="container">
				<div className="mentors__head">
					<h2 className="title title--h2 text-center">
